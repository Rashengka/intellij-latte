...
- A tag, filter or function the project's Latte does not have is reported as such rather than as unknown - "Function 'hasBlock' does not exist before Latte 3.0.10" rather than "not found": "Tag {includeblock} was removed in Latte 3.0", "Filter 'column' does not exist before Latte 3.1.3". "Unknown tag" sends the reader looking for a typo in a name they have spelled correctly; it stays for a name the reference tables never mention, and for a project whose version could not be established
...
- The module in a link - `Front` in `:Front:Home:default`, each of `Core` and `Front` in `:Core:Front:Home:` - is a link to the module's folder. It used to lead to a base presenter that happened to share the name, or nowhere. Nette puts every module in one whole part of the presenter's namespace, bare or with a suffix, and leaves folders to the autoloader, so the folder is found from the presenter the link targets: up from its file, as long as every folder on the way is named like its part of the namespace. That covers `App\Presentation\*\**Presenter`, `App\Modules\*\Presenters\*Presenter` and `App\*Module\Presenters\*Presenter` alike. Where it cannot be proven - a mapping for a single module leaves the module out of the namespace, a folder is named otherwise - the module is left without a link; the configuration itself is not read
- A tag the project's Latte deprecates is reported as deprecated: "Tag {includeblock} is deprecated since Latte 2.11", from the version the reference table names as the one where the engine starts warning. Under Latte 3 the same tag is reported as removed instead, and a project whose version could not be established is told neither. A deprecation that holds only in some places - `{first}`, `{last}` and `{sep}` outside a `{foreach}` from 3.1.6 - is not reported, because the tag alone does not say where it stands
...
- A link in the template of a component goes where Nette sends it. Inside a component every destination of `{link}` and `n:href` is a signal of that component - `loadMore` and `loadMore!` both lead to its `handleLoadMore()` - and a colon names a subcomponent, so `Home:default` there is not a presenter; `{control pager}` asks the component for `createComponentPager()`. The plugin used to answer all of them from a presenter whose name matched the component's, which it offered as confidently as a correct target. `{plink}` still links a presenter when it names one, and leads nowhere when it does not, because which presenter renders the component the template does not say. A template belongs to a component when its `{templateType}` is the component's `…Template`, or when a component, and no presenter, sits in the same folder; two components there make it a component's template without saying whose, and then the signal leads nowhere
...
- False "Undefined class" warning for `{\PHP_EOL}`, which prints a constant in every supported version. `{PHP_EOL}` without the backslash stays an unknown tag: a bare name standing as the whole tag is a tag name to Latte, and every supported version refuses it. A class name is now reported only where PHP requires one: in front of `::`, after `new` or `instanceof`, in a written-out type, and as the argument of `{templateType}`
